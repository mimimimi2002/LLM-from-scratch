...
class SimpleTokenizer:
  ...
  @classmethod
  def create_vocabulary(self, file_name):
    try:
      ...
      raise ValueError(f"cannot read {file_name} as UTF-8")
  ...
  def encode(self, text):
    ...
    # turn to tokenIDs
    return [self.word_to_token[s] if s in self.word_to_token else UNK for s in preprocessed]

  def decode(self, ids):
    text = " ".join([self.token_to_word[id] if id in self.token_to_word else "" for id in ids])
